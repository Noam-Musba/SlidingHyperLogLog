...
"""
BIN_HASH_SIZE is the size int bits of the hashed element returned from the hash function.
...
# the time at the start of the execution, mainly used for initializing the buckets.
START_TIME = time()
...
"""
starting to catch QUIC packets and adding them up until a Ctrl+C interruption is happening
"""
print("~~~adding phase~~~")
	...
	for packet in live_cap.sniff_continuously():
		for layer in packet.layers:
			if layer.layer_name == "quic":
				if layer.has_field("dcid"):  # extract the dcid (Destination Connection ID)
					cid = layer.get_field("dcid")
					arrival_time = time()
